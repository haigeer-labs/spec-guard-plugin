...
只动了 `evals/`、`scripts/` 和文档，插件内容没变——这部分本身不构成升版本的理由，
但它和下面的 0.7.22 在同一轮里做完。

补的是上一轮那份「接下来是什么」里排前两位的两件事。

### 新增

- **`evals/sync-map.sh`** —— 操作一（能力图落库）的行为验证。

  四个操作里只有操作三有 eval，操作四在 `sentinel-livelab` 真跑过 4 个 PR，
  操作一和操作二在 `docs/walkthrough.md` 真跑过 —— 但那是 0.5 时代，
  而 0.7.19（`dee3d81`）刚改了 Epic 正文和模块摘要的来源。**改完没跑过**，
  而操作一是唯一做不可逆外部写入的那个，也是所有人的第一步。

  `crash` 组验的是 **0.7.13 那条「每建成一个 issue 就立刻写回 state.json」** ——
  它至今只是 skill 里的一句话。桩在第 3 次建 issue 时失败，判据从日志算出
  「失败前建成了几个」再和 `state.json` 里的条目数比，不写死数。

  **首跑跑了三次才拿到有效结论，前两次都是评测自己的毛病：**

  1. 判据把模型查用法打的 `gh issue create --help` 数成了「第二个 Epic」，
     报了个假失败 —— 判据管得太宽和管得太窄一样是缺陷（lenses A3）。
  2. 桩也把 `--help` 当成一次建 issue：消耗掉 101 号、提前触发失败点，
     模型于是以为自己建了个孤儿 issue 并停下来问。那一轮 crash 组的结果
     整个是脚手架污染，不是产品结论。

  第三次全绿：Epic 1 个 / 3 个模块 issue 都挂在 Epic 下 / Epic 正文 227 字节
  vs 能力图 507 字节 / 2 条依赖边 / state.json 写全；crash 组 4 次建 issue、
  2 次被打断、建成 2 个，state.json 正好记了 2 个 —— **增量写回坐实**。

  `--selftest`（8 条，含「`--help` 不算一次建 issue」）已接进 `validate.sh`。
  它抓到过判决器自己的一个 bug：python 的 `len()` 数字符、`wc -c` 数字节，
  中文正文差 2~3 倍，「灌了全文」因此判不出来。

- **`scripts/install-git-hooks.sh`** —— pre-push 钩子。

  本仓的 GitHub Actions **从 v0.1.0 至今跑过 0 次**
  （`gh api .../actions/runs` → `total_count: 0`，账户级 Actions 被禁）。
  114 条 hook 断言、22 条校验器断言、shellcheck，全部依赖**一个人记得
  在自己机器上敲那三条命令**。

  钩子在 push 前跑 `validate.sh` + 两套 hook 断言（约 50 秒），红了就中止。
  每样只跑一遍（跑两遍取输出再取退出码会变成 100 秒，而慢到让人条件反射
  加 `--no-verify` 就等于没装）。已用「人为注入一条失败」验证过它真的拦得住。

  **这是止血带不是解药。** 真正的解法是恢复账户级 Actions ——
  在那之前，bash 3.2 那一层的唯一保障仍然是作者这台 Mac。

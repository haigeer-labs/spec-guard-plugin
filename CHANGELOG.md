...
- **`evals/sync-map.sh` 的判据会对着正确行为报失败。**

  它写的是「Epic 用了 `--body-file` → ❌，正是 0.7.19 修掉的那条」。
  但 0.7.19 修的规则是**「不要 `--body-file spec/CAPABILITY-MAP.md`」**——
  关键在指向的是谁，不在用了哪个 flag。判据实现成了后者。

  0.7.23 之前这个差别不显形（当时的 SKILL 恰好用 `--body`）。0.7.23 的正文是
  多行、含 HTML 注释标记，走 `--body` 的 shell 引号必炸，只能用
  `--body-file /tmp/epic-body.md` —— **旧判据于是把新的正确行为判成违规**。

  和 CHANGELOG 里记过的「新约定 + 旧检查器」是同一件事，只不过这次落在
  评测层，而且是插件作者自己的改动触发的。同一个形状第 N 次：
  **判据管得太宽和管得太窄一样是缺陷**（lenses A3）。

  改法是把判据从「用了哪个 flag」挪到「正文是什么内容」：`gh` 桩现在把
  `--body-file` 指向的**内容**一并记进日志（多记一对 `__BODYTEXT__ <正文>`），
  判据只在路径指向能力图本身时才报违规。

### 新增

- **`evals/sync-map.sh` 补上 0.7.23 那两层判据。** 它验的是操作一，而 0.7.23
  把操作一改得比 0.7.19 那次大得多（标记块、刷新分支、指纹写回）——
  判据不跟上，这个 eval 验的就是上一代的形态。

  - **标记块**：Epic 和每个模块 issue 的正文都得有 `<!-- BEGIN/END:spec-guard-sync -->`。
    没有标记 = 将来刷不回来 = 检测得出、修不了。
  - **指纹算得对**：不只查字段在不在，而是拿 `spec-digest.py` 重算一遍比对。
    「写了个假 hash」比不写更糟——要么永远报过期（关不掉的警报），要么永远
    不报（谎报已同步），而只查字段存在的判据抓不到它。
  - **crash 组**：号和指纹必须**同一次写入**。中途失败恰恰是最容易只写一半的时刻。

  `--selftest` 8 → 14 条（已接进 `validate.sh`）。新增的里有一对是专门钉住
  上面那个判据缺陷的：`--body-file` 指向临时文件**必须通过**、指向能力图
  本身**必须不通过**。少了前者，判据随时会退回「看 flag 不看内容」。

  **真跑通过（2026-08-29，0.7.23）**：happy 组 Epic 1 个 / 3 个模块 issue 全挂
  在 Epic 下 / 正文 241 字节 vs 能力图 507 字节 / 4 个正文都有标记块 /
  **指纹都写回了且算得对** / 2 条依赖边；crash 组 4 次建 issue、2 次被打断、
  建成 2 个，号和指纹都在 `state.json` 里。

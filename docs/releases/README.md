...
| --- | --- | --- | --- |
| Codex CLI | `source-verified`：adapter、hook 与 smoke 判决器回归 | `installed-verified` / `host-verified`（v0.13.0）：从 `v0.13.0` tag 安装，真实 smoke 收到阶段注入，见 [v0.13.0-codex.json](v0.13.0-codex.json) | 显式确认；模块严格串行推进 |
| Codex 桌面 | `source-verified`：共享 skill/hook 回归 | `not-verified`：无原生桌面 UI 观察记录 | 遵从桌面批准；模块严格串行推进 |
| Claude Code CLI | `source-verified`：命令、hook 与 bridge 回归 | `not-verified`：无指定新会话记录 | 显式确认；模块严格串行推进 |

...
## 第四次实跑:同一份数据回答了两个问题,而我只问了一个

**环境**:两个脚手架项目 + headless `claude -p` · 2026-08-27
**验的是**:0.7.0 那次瘦身赖以成立的假设 —— 模型真的会去加载 skill 吗

### 首跑（0.7.4）

| 组 | 配置 | 结果 |
|---|---|---|
| A | 15 行声明块 | ✅ 第 8 个工具调用时加载 skill |
| B | 无声明块（有 `state.json`） | ❌ 全程没加载,转头设计表结构、问技术栈 |

当时的判据写的是「A 加载 && B 不加载」,结论读成:**声明块起作用了**。收工。

### 两天没到就发现读漏了

B 组有 `.agent/state.json`,hook 照常激活 —— **B 组就是 `--no-claude-md`
零足迹模式**。也就是说这份数据同时说了第二件事:

> 那个我们已经发货、并在已知限制里写着「靠 hook 每轮兜底」的模式,**兜不住**。

原因很简单:**hook 注入的是「状态」,而让 skill 被加载的是那句「指令」。**
0.7.0 把指令留在了声明块里,而零足迹模式恰恰没有声明块。

### 修完复跑（0.7.5）

hook 在检测到「无声明块」时把触发指令补进注入内容。同一个脚手架、同一句话:

| 组 | 结果 |
|---|---|
| B | ✅ **第 1 个工具调用**就加载 skill(比 A 的第 8 个还早) |

随后的行为完全按约定:拒绝编造验收标准、明确拒绝建 `todo.md`
(「约定明令禁止的双真相源」)、提出开 `feat/identity` 模块分支,
还抓出 `spec/identity.md` 与两个 task 内容不一致。

### 三条教训

1. **「另一个机制会兜住」是最容易想当然的一类论断。** 它听起来像系统设计而不
   像假设,所以不会被当成待验证项。这条从 0.7.0 起挂在已知限制里,写的时候
   没验,一验就是反的。

2. **对照组不只是对照,它本身就是一个被测配置。** 设计 A/B 时该多问一句:
   「B 是不是我们也要发货的东西?」是的话,「B 表现差」就不是佐证 A 的证据,
   而是 B 的缺陷报告。

3. **判据会随产品变。** 修完 0.7.5 之后,原判据「B 必须不加载」立刻变成错的 ——
   差一点就留下一个**断言零足迹模式必须失效**的评测。改判据后拿旧数据做了
   反向回归:喂 0.7.4 时代的 transcript 必须判红,确认它有牙。

---

## 第五次实跑:头号卖点的第一次证据

**环境**:两个脚手架 + headless `claude -p`,判据是**文件系统** · 2026-08-27
**验的是**:README 问题① ——「四个模块递归跑下来,第二个模块的 `/plan` 会覆盖
第一个的 `tasks/plan.md`」

这条从立项起就写在 README 第一段,是整个插件存在的理由。**从没被行为验证过。**
单测只验了 hook 的状态机(文件在不在),没验模型拿到约定后**真的会不会**把产物
放进命名空间。

### 为什么挑 local 模式

- github 模式的两条通路第四次实跑已经验过
- **local 模式没有 skill 兜底**,13 行的块就是全部约定 —— 它不 work 就是真不 work
- local 是上游原生路径,新用户更可能从这儿进来

### 结果

同一句「为 identity 模块拆任务,产出计划和任务清单文件」:

| | 产物落点 | 命名空间 | 根下单例 |
|---|---|---|---|
| 有约定 | `tasks/identity/plan.md` + `tasks/identity/todo.md` | 2/2 | 0 |
| 无约定 | `tasks/plan.md` + `tasks/todo.md` | 0/2 | **2** |

**对照组精确复现了 README ① 描述的那个 bug。** 卖点成立。

### 这次的对照组是干净的

第四次实跑刚学到「对照组本身也是一个被测配置」。这次要说清楚:**这里的对照组
不是我们发的货**,它是「不装插件」的基线。所以「B 表现差」在这里确实是佐证 A
的证据,不是缺陷报告。

**两种对照组要分清**:
- 对照的是**我们也要发的另一个配置** → 它表现差 = 缺陷报告(第四次实跑)
- 对照的是**不装/不用**的基线 → 它表现差 = 卖点成立(这一次)

分不清的代价第四次已经付过了。

### 判据选文件系统而不是 transcript

模型可以在回答里把命名空间说得头头是道,然后写进 `tasks/plan.md`。
**看它做了什么,不看它说了什么** —— 这也是为什么这个评测比
`skill-deferral`(判 tool_use)更硬一档。

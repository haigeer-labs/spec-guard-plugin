...
成功时，用户可将插件根的 server、hooks 与 `manifest.json` 打包为 Claude Desktop 的 MCP Bundle（`.mcpb`），选择项目根后调用
与 CLI/Codex 同一套 phase、verify、history verify 和 GitHub/GitLab sync-map preview。

## Tech Stack

- Node.js 标准库：newline-delimited JSON-RPC 2.0 stdio server、输入校验与子进程调用；不新增
  npm 依赖。Claude Desktop 自带 Node.js，避免要求用户另装 Python。
...
node plugins/spec-guard/mcp/claude_desktop_server.mjs
```

## Project Structure

```text
plugins/spec-guard/mcp/claude_desktop_server.mjs   # stdio MCP server
plugins/spec-guard/manifest.json                    # MCPB bundle manifest
...
- JavaScript 标识符使用 camelCase；工具名使用 MCP kebab-free snake_case。
...
- 回归：模拟不存在路径、非 Git 目录、缺少 glab/gh、损坏 state 与 hook 非零退出；均返回可读
  `isError` 结果而不崩溃。
- 手动：安装 Claude Desktop MCPB，列出工具并执行一次 `phase`；写工具不可出现。

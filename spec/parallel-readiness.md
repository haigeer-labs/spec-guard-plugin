...
- 依赖层只由 `Depends on` 计算；`Build order` 的并列组用于严格拓扑验证与稳定展示，
  线性展示也不会凭空增加依赖。
...
`Build order` 必须包含且仅包含所有模块，并满足每条依赖边的拓扑顺序；逗号表示同一声明组，
组内依赖同样无效。严格输入消费者使用共享解析器的声明组/展开顺序；摘要仍保留表格行序。
不符合时报告图无效，不给出候选组。此严格性防止“看似可并行”的建议来自坏图。
...
5. 输出不含任何“已经安全并行”或“可立即执行”的结论，也不触发宿主任务生命周期操作。

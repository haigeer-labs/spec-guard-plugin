...
- Python 3 标准库：解析严格 JSON 边界声明、规范化相对路径、计算交集、输出稳定 JSON。
...
- `paths` 仅接受无通配符、无 `..`、仓库根相对的文件或目录路径；目录拥有权与其任一
  后代路径重叠即冲突。声明的新文件仍写预期相对路径。
...
- `manual-parallel-eligible`：每个模块均有完整声明，路径/API 没有交集，且迁移、全局
  配置、测试资源均明确为空。仍须用户确认后才可人工启动隔离工作区。
...
- 安全：路径逃逸、绝对路径、通配符与重复标题被拒绝；不读取或修改 state、Issue、分支或
  worktree。
